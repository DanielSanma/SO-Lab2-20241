...
char *path_directories[MAX_ARGS];
...
int main(int argc, char **argv) {
    ...
    if (argc > 2) {
        wish_error("Error: Too many arguments");
        exit(1);
    } else if (argc == 2) {
        // Batch mode
        FILE *batch_file = fopen(argv[1], "r");
        if (batch_file == NULL) {
            wish_error("Error: Cannot open batch file");
            exit(1);
        }
...
char **wish_split_line(char *line) {
    ...
    if (!tokens) {
        wish_error("Error: Memory allocation failed");
        exit(1);
    }
...
int wish_execute(char **args) {
    ...
    // Si el comando no es interno, ejecutarlo en el proceso padre
...
int wish_launch(char **args) {
    ...
    // Verificar si el comando es un cambio de directorio
    ...
    // Si el comando no es un cambio de directorio, ejecutarlo en el proceso padre
    if (out_redirect_index >= 0) {
        ...
        for (int i = out_redirect_index + 1; args[i] != NULL; i++) {
            ...
            if (output_file == NULL) {
                ...
                wish_error("Error: Cannot open output file");
                return 1;
            }
        }
    ...
    for (int i = 0; path_directories[i] != NULL; i++) {
        ...
        if (access(path, X_OK) == 0) {
            ...
            wish_error("Error: Execution failed");
            return 1;
        }
    }
